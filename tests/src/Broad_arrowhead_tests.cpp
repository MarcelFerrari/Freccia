...
Eigen::MatrixXd generateTBDMatrix(unsigned int N, unsigned int f){
    Eigen::MatrixXd A = Eigen::MatrixXd::Zero(N+f, N+f);
    ...
    // Populate arrowhead
    {
    Eigen::MatrixXd W = Eigen::MatrixXd::Random(N, f) * 10;

    A.topRightCorner(N, f) = W;
    A.bottomLeftCorner(f, N) = W.transpose();
    }
    
    // Populate arrow tip
    {
    Eigen::MatrixXd W = Eigen::MatrixXd::Random(f, f) * 10;
    W *= W.transpose();
    A.bottomRightCorner(f, f) = W;
    }
...
int test_input(unsigned int t, Eigen::MatrixXd& A, unsigned int f){ 
    // Set up matrix in compressed format
    Eigen::MatrixXd B = Freccia::Banded::make_compressed(A.topLeftCorner(A.rows()-f, A.cols()-f), f+1);
    Eigen::MatrixXd W = A.rightCols(f);
    
    unsigned int N = B.cols() + W.cols();

    // Compute eigendecomposition
    Freccia::Arrowhead::BroadArrowheadEigenSolver solver(B, W, 'L');
